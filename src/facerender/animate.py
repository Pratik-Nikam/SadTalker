...
from src.facerender.modules.make_animation import make_animation, make_animation_streaming, keypoint_transformation 
...
class AnimateFromCoeff():
    ...
    def generate(self, x, video_save_dir, pic_path, crop_info, enhancer=None, background_enhancer=None, preprocess='crop', img_size=256):
        ...
        # imageio.mimsave(path, result, format='FFMPEG')
    ...
    def generate_streaming(self, x, video_save_dir, pic_path, crop_info, enhancer=None, background_enhancer=None, preprocess='crop', img_size=256, callback=None):
        """Streaming version that yields frames as they're generated"""
        import time
        import threading
        ...
        def create_nodding_frame():
            """Create a simple nodding frame from the source image"""
            # Simple head rotation for nodding effect
            nodding_semantics = source_semantics.clone()
            # Add slight pitch variation for nodding - more realistic range
            if nodding_semantics.shape[1] > 6:  # Check if pitch is available
                nodding_semantics[:, 6] += 0.05 * np.sin(time.time() * 2)  # Gentle nod
            return nodding_semantics
        
        # Show initial nodding animation
        if callback:
            try:
                nodding_semantics = create_nodding_frame()
                ...
                print(f"Error in nodding animation: {e}")
                # Continue without nodding animation
        
        # Stream actual frames
        for frame_pred, progress in make_animation_streaming(
            source_image, source_semantics, target_semantics,
            self.generator, self.kp_extractor, self.he_estimator, self.mapping, 
            yaw_c_seq, pitch_c_seq, roll_c_seq, use_exp=True, callback=callback
        ):
            predictions_video.append(frame_pred)
