...
class SadTalker():
    ...
    def test_streaming(self, source_image, driven_audio, preprocess='crop', 
        still_mode=False,  use_enhancer=False, batch_size=1, size=256, 
        pose_style = 0, exp_scale=1.0, 
        use_ref_video = False,
        ref_video = None,
        ...
        use_idle_mode = False,
        length_of_audio = 0, use_blink=True,
        result_dir='./results/', callback=None):
        """Streaming version that yields progress and frames as they're generated"""
        ...
        #coeff2video with streaming
        data = get_facerender_data(coeff_path, crop_pic_path, first_coeff_path, audio_path, batch_size, still_mode=still_mode, preprocess=preprocess, size=size, expression_scale = exp_scale)
        
        # Use streaming generation
        return_path = self.animate_from_coeff.generate_streaming(
            data, save_dir, pic_path, crop_info, 
            enhancer='gfpgan' if use_enhancer else None, 
            preprocess=preprocess, img_size=size, callback=callback
        )
